...
synch_type = 0; % 0: none; 1: deaths; 2: confirmed cases
...
%%
meddays = 4;
plot_country = {'Italy','United Kingdom','Hubei','US','ChinaAll'}
col = 'gbrcykrc'
pop = [60.5e6 66.4e6 58.5e6 372e6 1.38e9 ]
deaths = deaa;
newdeaths = medfilt2(diff(deaths,1,2),[1 meddays],'symmetric')

figure
hold all
for ic=1:length(plot_country)
    idx = find(strcmp(countries,plot_country{ic}))
    plot(deaths(idx,2:end-1)'./pop(ic),newdeaths(idx,1:end-1)'./pop(ic),[col(ic) '-'],'linewidth',3)
    plot(deaths(idx,end-1)'./pop(ic),newdeaths(idx,end-1)'./pop(ic),[col(ic) '.'],'markersize',50)
end
set(gca,'xscale','log','yscale','log')
plot((1e-9:1e-6:5e-4),.3*(1e-9:1e-6:5e-4),'k')
legend([reshape(repmat(plot_country,[2 1]),1,[]),'ref'],'location','eastoutside')
box on
axis square

%%
meddays = ;
plot_country = {'Italy','United Kingdom','Germany','France','Spain'}
col = 'gbrcykrc'
pop = [60.5e6 66.4e6 82.8e6 67e6 46.7e6]
deaths = deaa;
newdeaths = medfilt2(diff(deaths,1,2),[1 meddays],'symmetric')
...
%%
meddays = 4;
plot_country = {'Italy','Sweden','Denmark','Netherlands','Belgium'}
col = 'gbrcykrc'
pop = [60.5e6 10.1e6 5.6e6 17.2e6 11.4e6]
deaths = deaa;
newdeaths = medfilt2(diff(deaths,1,2),[1 meddays],'symmetric')
